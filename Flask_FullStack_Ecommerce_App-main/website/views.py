...
def home():
    ...
    sqlcommand = 'SELECT * FROM FASHION LIMIT 4, 10'
    cursor.execute(sqlcommand)
...
def process_sort_form():
    ...
    match selected_option:
        case "Default Sort":
            sqlcommand = ("select * from FASHION")
        case "Sort By Price":
            sqlcommand = ("select * from FASHION ORDER BY CAST(REPLACE(REPLACE(price, '$', ''), '.', '') AS INT)")
        case "Sort By Price DESC":
            sqlcommand = ("select * from FASHION ORDER BY CAST(REPLACE(REPLACE(price, '$', ''), '.', '') AS INT) DESC")
        case "Sort By Rating":
